...
<template>
  ...
  />
  <!-- Mantis: emerald/green accent, grouped with collapsible-style headers -->
  <aside
    id="mantis-sidebar"
  ...
  >
    <div class="flex min-h-0 flex-1 flex-col">
      <div class="shrink-0 px-4 py-4">
        <NuxtLink to="/" class="flex items-center gap-2 font-semibold text-emerald-600 dark:text-emerald-400">
          <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-500/15 text-emerald-600 dark:bg-emerald-500/25">M</span>
          <span>Mantis</span>
        </NuxtLink>
      </div>
      <nav class="flex-1 overflow-y-auto px-3 pb-4">
        <div v-for="section in sections" :key="section.title" class="mb-4">
          <div class="mb-1 flex items-center gap-2 px-3 py-1.5">
          ...
          <ul class="space-y-0.5">
            <li v-for="item in section.items" :key="item.path">
              <div
                ...
                class="flex items-center gap-1 rounded-lg px-3 py-2 text-sm"
                :class="props.isActive(item.path)
                  ? 'bg-emerald-500/15 font-medium text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300'
