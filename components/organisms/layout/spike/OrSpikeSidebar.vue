...
<template>
  ...
  />
  <!-- Spike: compact list, blue accent, active = left bar + bg -->
  <aside
    id="spike-sidebar"
  ...
  >
    <div class="flex min-h-0 flex-1 flex-col p-3">
      <div class="mb-5">
        <NuxtLink to="/" class="flex items-center gap-2 font-semibold text-blue-600 dark:text-blue-400">
          <span class="flex h-7 w-7 items-center justify-center rounded-md bg-blue-600 text-white text-xs font-bold">S</span>
          <span class="text-sm">Spike</span>
        </NuxtLink>
      </div>
        ...
        <ul class="space-y-0.5">
          <li v-for="item in props.navigation" :key="item.path">
            <div
              ...
              class="flex items-center gap-1 rounded-md border-l-2 py-2 pl-3 pr-2"
              :class="props.isActive(item.path)
                ? 'border-blue-600 bg-blue-500/10 font-medium text-blue-700 dark:border-blue-400 dark:bg-blue-500/20 dark:text-blue-300'
                : 'border-transparent text-gray-600 dark:text-gray-400'"
            >
              <NuxtLink
                :to="item.path"
                class="flex min-w-0 flex-1 items-center gap-2.5 text-sm"
                @click="emit('update:open', false)"
              >
            ...
            <NuxtLink
              v-else
              :to="item.path"
              class="flex items-center gap-2.5 rounded-md border-l-2 py-2 pl-3 pr-2 text-sm transition-colors"
              :class="props.isActive(item.path)
                ? 'border-blue-600 bg-blue-500/10 font-medium text-blue-700 dark:border-blue-400 dark:bg-blue-500/20 dark:text-blue-300'
                : 'border-transparent text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'"
              @click="emit('update:open', false)"
            >
              <component :is="item.icon" class="size-4 shrink-0" />
              <span>{{ item.name }}</span>
