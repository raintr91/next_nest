...
<template>
  ...
  />
  <!-- Flyonui: teal/emerald accent, active = rounded icon bg + text -->
  <aside
    id="flyonui-sidebar"
  ...
  >
    <div class="flex min-h-0 flex-1 flex-col p-4">
      <div class="mb-6">
        <NuxtLink to="/" class="flex items-center gap-2 font-semibold text-teal-600 dark:text-teal-400">
          <span class="flex h-8 w-8 items-center justify-center rounded-xl bg-teal-500/15 text-teal-600 dark:bg-teal-500/25">F</span>
          <span>Flyonui</span>
        </NuxtLink>
      </div>
        ...
        <ul class="space-y-1">
          <li v-for="item in props.navigation" :key="item.path">
            <div
              ...
              class="flex items-center gap-1 rounded-lg px-3 py-2.5"
